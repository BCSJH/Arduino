...
String datas = "";
  ...
  void loop() 
  {
    Wire.requestFrom(SLAVE, 1);//우노끼리 통신
    String input ="";
    ...
    if(datas == "2" | input== "2"){ //현관문 ON
        sound_on();
        datas = "";
    }
    if(datas == "6" | input== "6"){ //현관문 OFF
        sound_off();
        datas = "";
    } 
    
    if (s.available()>0){//와이파이 제어 값을 받았을 때
        char c = s.read();
        Serial.print("data_WIFI : ");
        Serial.println(c);
    }
